...
% [~,rfname] = fileparts(fileparts(fpath));
% rfpath = fullfile(fileparts(fileparts(fileparts(fpath))),'Data',rfname);
% r_us_idx = find(fname == '_',2,'last');
% rfname = [fname(1:r_us_idx(1)) 'refimg.mat'];
% %% %
% refIm = load(fullfile(rfpath,rfname)); refIm = refIm.refimg; % Load reference image
% ================HX version load data===================
[HX_file, HX_path] = uigetfile('*.tif');
HX_filename = [HX_path filesep HX_file];
imageinfo = imfinfo(HX_filename,'tiff');
numframes = length(imageinfo);
H=540; W=640;
refIm = imread(HX_filename,'tiff',1,'Info', imageinfo);
% =======================================================
...
% % %
close all
imagereg = imwarp(refIm,tform,'OutputView',imref2d(size(dorsalMaps.dorsalMapScaled)));
figure
imagesc(imagereg);
...
%dm_us_idx = find(fname == '_',2,'last');
%dmname = [fname(1:dm_us_idx(1)) 'dorsalMap.mat'];
splitted = split(HX_file,'.');
dmname = [splitted(1) 'dorsalMap.mat'];
if saveMap == 1
    save(char(fullfile(HX_path,join(dmname,'_'))),'dorsalMaps');
...
load('D:\Scripts\allen_map\allenDorsalMap.mat');
...
set(gcf,'Position',[10 -273 1268 954])
subplot(221);
...
handle=subplot(222);
imagesc(image); axis equal off; colormap gray; hold on;
[x,y] = getline(handle);
OBpoints=[x y];
