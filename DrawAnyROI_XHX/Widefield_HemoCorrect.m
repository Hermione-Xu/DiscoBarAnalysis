...
% X and Y are dimensions in pixel space.
...
if size(data,3) - size(hemoData,3) == 1
    last_frame = data(:,:,end);
    data = data(:,:,1:end-1);
elseif size(hemoData) - size(data,3) == 1
    hemoData = hemoData(:,:,1:end-1);
end
data = single(data); 
[A,B,C] = size(data);
...
if isa(data,'gpuArray') % gpuArray = array stored on GPU
...
if exist('last_frame','var') == 1
    data = cat(3,data,last_frame);
end