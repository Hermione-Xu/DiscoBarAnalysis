...
% paths for saving tiffs
addpath(genpath('/opt/Fast_Tiff_Write')); % add fast tiff write

BlueTif     = fullfile(BinaryPath,'470_binned_1.tif');
VioletTif   = fullfile(BinaryPath,'407_binned_1.tif');
Btif        = Fast_Tiff_Write(BlueTif,1,1);
Vtif        = Fast_Tiff_Write(VioletTif,1,1);
nexttiff    = 2*(BinBy^2);
...
for i = 1:nStacks % for every raw tiff stack
    ...
    tic % 4.1 - much slower
    for f = 1:numel(blue_indices)
        Btif.WriteIMG(uint16(BinnedStack(:,:,blue_indices(f))));
    end
    for f = 1:numel(violet_indices)
        Vtif.WriteIMG(uint16(BinnedStack(:,:,violet_indices(f))));
    end
    toc
